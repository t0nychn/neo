...
class Tests:
    ...
    def test_normal_args(self):
        ...
        assert find_discount('£15 OFF!') == {'£15 OFF'}
        assert find_discount('25% Off Your Next Visit') == {'25% Off'}
        # test for phrase variations
        assert find_discount('5% Discount') == {'5% Discount'}
        assert find_discount('10% discount') == {'10% discount'}
        # test multiple instances in text
        assert find_discount('20%off 50 % off') == {'20%off', '50 % off'}
        assert find_discount('£20 off and a further 50% off') == {'£20 off', '50% off'}
        assert find_discount('20% discount and 10% off') == {'20% discount', '10% off'}
        # test on real data
        df = pd.read_csv('sites_data/initial.csv', index_col='id')
        assert find_discount(df.loc[8]['content']) == {'15% off'}
        assert find_discount(df.loc[53]['content']) == {'25% off'}